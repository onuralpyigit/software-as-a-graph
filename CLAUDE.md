...
| `cm_avg_cbo` and `cm_avg_rfc` show up in the UI and in exported graphs but never move a criticality score | Ingested, flattened and persisted, but no scoring code reads them — `CQP(v)` only consumes `loc`, `cyclomatic_complexity`, `instability_code` (from `avg_fanin`/`avg_fanout`), and `lcom`. See [structural-analysis.md §11.2](docs/structural-analysis.md#112-rm-formulas). |
| REV, RCL, and `w_in`/QADS (Reverse Eigenvector, Reverse Closeness, QoS-weighted In-Degree) are still computed by `StructuralAnalyzer` and shown in exported graphs, but no RM formula reads them | Same class of gotcha as the row above — they were `V(v)`'s three terms before Vulnerability/Security was retired outright in the RMAV→RM migration. See [structural-analysis.md §11.4](docs/structural-analysis.md#114-metric-orthogonality). |
