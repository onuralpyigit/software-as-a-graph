...
| `cm_avg_cbo` and `cm_avg_rfc` show up in the UI and in exported graphs but never move a criticality score | Ingested, flattened and persisted, but no scoring code reads them — `CQP(v)` only consumes `loc`, `cyclomatic_complexity`, `instability_code` (from `avg_fanin`/`avg_fanout`), and `lcom`. See [structural-analysis.md §11.2](docs/structural-analysis.md#112-rmav-formulas). |
