...
- **Closed-loop counterfactual verification.** Step 6 (`EditVerifier`) accepts candidate edits $\Delta(G)$ only if the counterfactual failure-impact reduction exceeds seed noise ($\Delta I > \kappa \cdot \sigma_{\text{seed}}$) across *every* propagation threshold. Enforced by `tests/test_prescription.py`.
