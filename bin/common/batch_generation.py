...
Generates one JSON dataset per scenario YAML found in input/, optionally
generates multi-seed variants for validation stability, and refreshes the
legacy input/system.json and input/dataset.json with current-format fields
...
Options:
    --input-dir PATH      Directory containing scenario_*.yaml   [default: input/]
    --output-dir PATH     Output directory for generated JSON     [default: output/]
    --refresh-legacy      Regenerate input/system.json and input/dataset.json
...
def add_batch_arguments(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--refresh-legacy", action="store_true",
                   help="Regenerate input/system.json and input/dataset.json")
...
def run_batch_generation(args: argparse.Namespace) -> int:
    ...
    if args.refresh_legacy:
        ...
        legacy_system = _PROJECT_ROOT / "input" / "system.json"
        records.append(generate_from_scale(
            scale="medium", seed=42,
            output_path=legacy_system,
            label="input/system.json (medium, seed=42)",
            force=True,
            dry_run=args.dry_run,
            verbose=args.verbose,
        ))
        # dataset.json — small scale, seed 42  (replaces stale version)
        legacy_dataset = _PROJECT_ROOT / "input" / "dataset.json"
        records.append(generate_from_scale(
            scale="small", seed=42,
            output_path=legacy_dataset,
            label="input/dataset.json (small, seed=42)",
            force=True,
            dry_run=args.dry_run,
            verbose=args.verbose,
        ))
        ...
        _warn("Skipping legacy refresh  (pass --refresh-legacy to regenerate "
              "input/system.json and input/dataset.json with code_metrics + system_hierarchy)")
