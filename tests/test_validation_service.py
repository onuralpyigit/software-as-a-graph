...
from saag.validation.models import LayerValidationResult, ValidationResult, ValidationTargets
from saag.validation.validator import Validator
...
class TestValidationService:
    ...
    def test_dimensional_validation_includes_all_metrics(self, validation_service, mock_prediction_service, mock_simulation_service):
        ...
        # Check Reliability
        rel = layer_res.dimensional_validation["reliability"]
        assert "ccr_5" in rel
        assert "cme" in rel
        
        # Check Maintainability
        mnt = layer_res.dimensional_validation["maintainability"]
        assert "cocr_5" in mnt
        assert "weighted_kappa_cta" in mnt
        assert "bottleneck_precision" in mnt
