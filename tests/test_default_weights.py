...
def test_quality_weights_defaults_are_ahp_derived():
    ...
    assert w.q_security == pytest.approx(0.16)
    
    # Check sum
    assert (w.q_availability + w.q_reliability + w.q_maintainability + w.q_security) == pytest.approx(1.0)
