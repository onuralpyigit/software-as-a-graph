...
```
A(v) = 0.25 × AP_c_directed(v) + 0.20 × QSPOF(v) + 0.20 × BR(v) + 0.25 × CDI(v) + 0.10 × w(v)
```

| Term | Weight | Rationale |
|------|--------|-----------|
| AP_c_directed(v) | 0.25 | Directed articulation score — primary structural SPOF baseline; identifies bottlenecks regardless of operational priority |
| QSPOF(v) | 0.20 | QoS-weighted SPOF severity — `AP_c_directed × w(v)`; highlights high-priority structural SPOFs |
| BR(v) | 0.20 | Bridge ratio — fraction of incident edges that are bridges; identifies irreplaceable structural foundations |
| CDI(v) | 0.25 | Connectivity degradation — path elongation upon removal catches non-SPOF availability risk. v4 promoted this to parity with `AP_c_directed`: CDI is now computed for every node in the main component rather than only articulation points, so it carries continuous SPOF-adjacent signal |
| w(v) | 0.10 | Component QoS weight — direct operational priority focus for availability focus |
