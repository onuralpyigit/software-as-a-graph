...
**No accuracy claim attaches to the weighting.** A sensitivity sweep withdrew the "AHP calibration adds accuracy" claim outright ([structural-analysis.md §11.6](structural-analysis.md#116-weight-shrinkage-strategy)). The decomposition is retained as an **attribution** mechanism — Reliability and Maintainability have distinct remedies and distinct owners — not as a ranking-improvement device. An equal-weight baseline ($w_R = w_M = 0.5$, $\alpha = 0.5$) is available via `--equal-weights`.
