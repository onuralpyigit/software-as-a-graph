...
- **Pure Structural Mode (`cli/analyze_graph.py`)**: Computes the 53-field structural metric vector $M(v)$ and graph summary $S(G)$ only. Fast, lightweight, and completely decoupled from scoring models. (This directly feeds Step 3's Pathway B and Step 4's Pathway A).
