...
**Figure 1**: The `saag` Prediction Step. Relation-specific graph embeddings ($\mathbf{h}_v$, §3.3.1) are decoded by two per-dimension multi-task heads (reliability, maintainability) and a composite head (§3.3.2) to produce the predicted criticality score $\hat{\mathcal{C}}(v)$. Every quantity in this figure is a function of $\mathbf{h}_v$ alone — see the note on RM below.
...
**Relationship to the deterministic RM framework.** `saag` also implements a separate, non-learned RM composite score — a closed-form, declared-weight function of structural metrics (fan-out, articulation points, betweenness, etc.; composite $Q(v)=0.80R(v)+0.20M(v)$, $R(v)$ itself a hierarchical blend of Fault Tolerance and Availability sub-scores, not AHP-weighted at the composite level), used elsewhere in the framework for anti-pattern detection and CI/CD reporting. An earlier internal pipeline blended this score into $\hat{\mathcal{C}}(v)$ at inference time; that blending step was found to be undisclosed in this paper's methodology and was removed prior to the results reported in §5 (§6.2 traces this in detail). $\hat{\mathcal{C}}(v)$ as evaluated in this paper is the pure network output defined above — the RM composite contributes to none of the numbers in §5. (This framework's composite was 4-dimensional, named RMAV, at the time the ensemble-removal history in §6.2 below took place; it has since migrated to the 2-dimensional RM model described here.)
...
For reference, the deterministic, non-learned RM composite baseline (§3.3, "Relationship to the deterministic RM framework") reaches mean $\rho=0.0584$ under the same $k$-fold protocol — HGL-QoS exceeds it by $\Delta\rho=+0.5287$. `TODO(needs re-measurement)`: this baseline figure was measured under the retired four-dimension RMAV composite and has not been reproduced under the two-dimension RM composite this session.
