...
These factors are also relevant to computational sustainability. Conducting architectural analysis solely on manifests eliminates the need for provisioned staging clusters, active containers, or live fault-injection harnesses, consequently minimizing deployment infrastructure overhead. Nevertheless, pre-deployment static analysis does not surpass simulation in speed: deterministic topological feature extraction requires $79.3\,\text{s}$ for a 520-component enterprise mesh and $239.3\,\text{s}$ for 2,000 components. By comparison, the in-process discrete-event cascade simulator completes in $0.08$–$4.5\,\text{s}$ (§§7.5.1 and 8.2). Although the learned GNN forward pass is minimal ($56\,\text{ms}$), cold static analysis is roughly eighteen times slower than in-process simulation, showing that pre-deployment static analysis provides an infrastructure-avoidance benefit rather than an in-process CPU execution advantage unless paired with incremental topology caching.
...
Previous research on dependability and performance has primarily emphasized runtime mechanisms, such as dynamic consensus, broker clustering, adaptive backpressure throttling, autoscaling, and automated failover. Concurrently, chaos engineering and runtime fault injection [23] introduce disruptions into live staging or production clusters to observe degradation and recovery. Although runtime testing provides realistic operational validation, it requires fully provisioned clusters, risks service disruption, and incurs substantial compute overhead. This limits its utility during early architectural design and lightweight commit-level continuous integration (CI/CD), situating such workflows among development-time computations scrutinized in green software engineering [29, 30, 31, 32, 33]. In contrast, this study investigates pre-deployment dependability analysis directly from Architecture-as-Code descriptors before provisioning runtime infrastructure, eliminating cluster provisioning overhead and preempting production restart storms.
...
The outer split $(\alpha_{\text{top}}, \beta_{\text{top}}, \gamma_{\text{top}})$ is a convex combination. Sweeping the full simplex leaves induced orderings intact ($\rho \ge 0.919$), moving downstream ranking by only $0.031$ (`Topo-QoS`) and $0.007$ (RM). Under Morris screening none of the three constants is load-bearing ($\mu^* \le 0.025$). The split is therefore a documented convention rather than a sensitive parameter.
...
Rule 6 is the only symmetric rule: two brokers colocated on the same host share that host’s physical failure domain ($w = w_V(\text{host})$). Colocated brokers compete for resources, and host crashes halt them simultaneously. Rule 6 applies in four of eight detection benchmark contexts (12 directed edges). Because simulation oracles operate strictly on $G_{\text{structural}}$ (§4.4), Rule 6 has zero influence on ground-truth failure labels.
...
-   **Reliability ($R(v)$):** Blends Fault Tolerance and Availability: $R(v) = r_{\text{FT}} \cdot FT(v) + (1 - r_{\text{FT}}) \cdot A(v)$ with $r_{\text{FT}} = 0.36$. Intra-dimension weights apply $\lambda = 0.70$ shrinkage blending with a uniform prior. Because comparison matrices are rank-one by construction, their Consistency Ratios are uninformative (see Supplementary §S4). Elicited AHP weights rank worse than a uniform prior against dynamic simulation (§7.3); pending formal user studies, we explicitly recommend the uniform prior.
...
The evaluation corpus comprises 2,812 components across seventeen system architectures, including twelve synthetic topologies that form the inductive cross-validation folds and five real-world reference systems withheld from all training procedures, as detailed in Table 5. The synthetic scenarios span diverse operational domains (autonomous vehicles, financial trading, healthcare integration, industrial SCADA, smart-city IoT, telecom RAN, cloud microservices, and enterprise application integration via centralized broker hubs/ESB; detailed in Table S11 of the Supplementary Material).
...
To assess generalization outside the generator, `HGT-QoS` was trained on all twelve synthetic scenarios and evaluated zero-shot across five open-source systems versus $I^*(v)$ (five seeds; evaluated in an exploratory 2-layer, 150-epoch configuration with within-graph rank normalization to prevent over-smoothing across the smaller topological diameters of these transcribed meshes, where $|V_{\text{app}}| \le 41$; no open-source graphs contributed training gradients or checkpoint selection). Table 11 reports the transfer performance, visually partitioned by communication paradigm into asynchronous pub-sub architectures and synchronous microservice call trees.

**Table 11.** Zero-shot out-of-family topological transfer to five open-source systems partitioned by communication paradigm, scored against $I^*(v)$ on the Application population ($V_{\text{app}}$), reporting both Spearman rank correlation ($\rho$) and critical-set identification ($F_1@K$ at $K = \text{round}(0.20 \cdot |V_{\text{app}}|)$). For **asynchronous pub-sub systems**, active-stratum rank correlation $\rho_{>0}$ is consistently positive ($+0.262$ to $+0.770$, subtotal mean $+0.516$). For **synchronous microservice call trees**, backward cascade dynamics via RPC timeouts establish an architectural boundary condition where active ranking collapses or inverts ($\rho_{>0} \le +0.031$, subtotal mean $-0.071$). `HGT-QoS` trains on all twelve synthetic scenarios ($\pm$ = spread over five seeds); RM, Topo, and `Topo-QoS` are training-free and are evaluated on identical labels and components. Where open-source systems lack declared QoS manifests, default middleware contract profiles (e.g., ROS 2 Best-Effort/Reliable, MQTT QoS 0/1) were applied uniformly across edges. **$\rho_{>0}$** restricts rank correlation to the $n_{>0}$ actively propagating components (§7.1.2). Bold indicates the best score per row.

|                                                                                                                            |                        |              |        |           |           |           |              |           |                       |                      |                       |
|:---------------------------------------------------------------------------------------------------------------------------|-----------------------:|-------------:|:------:|:---------:|:---------:|:---------:|:------------:|:---------:|:---------------------:|:--------------------:|:---------------------:|
| **Real-World Architecture**                                                                                                | **$|V_{\text{app}}|$** | **$n_{>0}$** | **RM** |           | **Topo**  |           | **Topo-QoS** |           |      **HGT-QoS**      |                      |                       |
|                                                                                                                            |                        |              | $\rho$ |   $F_1$   |  $\rho$   |   $F_1$   |    $\rho$    |   $F_1$   |     $\rho$ (Full)     | $\rho_{>0}$ (Active) |        $F_1@K$        |
| ***Asynchronous Pub-Sub Systems** (Forward failure cascades; consistently positive active $\rho_{>0} \in [+0.26, +0.77]$)* |                        |              |        |           |           |           |              |           |                       |                      |                       |
...
| *Pub-Sub Subtotal Mean*                                                                                                    |                      — |            — | 0.364  |   0.122   |   0.379   |   0.122   |    0.400     |   0.122   |       **0.806**       |  $\mathbf{+0.516}$   |       **0.589**       |
| ***Synchronous Microservice Call Trees** (Architectural boundary condition: backward cascade dynamics invert ranking)*     |                        |              |        |           |           |           |              |           |                       |                      |                       |
...
| *Synchronous Subtotal Mean*                                                                                                |                      — |            — | 0.745  |   0.438   |   0.710   |   0.438   |    0.715     |   0.438   |       **0.772**       |       $-0.071$       |         0.450         |
| **Overall Mean**                                                                                                           |                      — |            — | 0.516  |   0.248   |   0.511   |   0.248   |    0.526     |   0.248   |       **0.792**       |   $+$0.281   |       **0.533**       |
...
2.  **Communication paradigm governs active transfer: pub-sub succeeds, synchronous call trees delineate the boundary.** Restricting to components with strictly positive impact costs `HGT-QoS` $0.511$ of correlation overall, leaving $\rho_{>0} = +0.281$, while all three training-free baselines turn negative ($-0.055$ to $-0.092$; Table 12). Partitioning by communication paradigm (Table 11) resolves the underlying mechanism: on **Asynchronous Pub-Sub Systems**, active $\rho_{>0}$ is consistently positive ($+0.262$ to $+0.770$, subtotal mean $+0.516$), with Home Assistant ($+0.770$), Autoware ($+0.516$), and EdgeX ($+0.262$) demonstrating genuine out-of-family transfer. In contrast, on **Synchronous Microservice Call Trees**, active correlation collapses to chance on Cloud Microservices ($+0.031$) and inverts on Train-Ticket ($-0.173$), yielding a negative subtotal mean ($-0.071$). **We therefore report RQ4 as a qualified positive:** relational learning successfully transfers on asynchronous event-driven architectures, but synchronous microservices expose a sharp architectural boundary condition.
...
This study investigated the empirical viability and boundaries of heterogeneous graph neural networks for pre-deployment dependability analysis in asynchronous distributed systems. Using Software-as-a-Graph (SaG) to transform Architecture-as-Code manifests into typed multigraphs, we evaluated a relation-specific Heterogeneous Graph Transformer with Quality-of-Service edge encodings against unparameterized structural baselines and an ISO/IEC 25010/25019 Reliability–Maintainability attribution layer without runtime telemetry.
...
These findings carry direct practical implications: teams requiring scalar rankings should consider the simpler untyped QoS-weighted baseline ($\rho = 0.604$ at $28{,}168$ parameters vs. HGT-QoS’s $0.638$ at $434{,}620$), as relational attention is narrow and degree-sensitive (§§7.3.3 and 8.1). Boundary conditions are explicit: learned ranking does not significantly outperform unparameterized QoS centrality ($+0.085$, $p = 0.151$), zero-shot transfer on active components is weak for all predictors and only the typed model stays positive ($+0.281$ vs. negative baselines, still inverting on synchronous call trees), prediction dispersion fails as an OOD indicator, and elicited AHP weights rank poorer than a uniform prior.

SaG provides a reproducible multigraph formulation of publish-subscribe systems, a byte-identically regenerating corpus, and an empirical evaluation of where architectural inductive biases compose. While neural inference executes in $56\,\text{ms}$, deterministic feature extraction is roughly eighteen times slower than simulation unless incremental caching is applied across commits. Validating whether such pre-deployment pipelines predict live production outages remains the central empirical frontier.
