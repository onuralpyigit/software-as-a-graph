...
One result governs how every other number in this paper is read. Measured against $I_{\text{comp}}(v)$ over the eight scenarios of the detection benchmark, stratified RM rank correlations are $\rho = 0.571$ (Application), $0.119$ (Broker), and $0.209$ (Node), while pooling all types collapses the correlation to $\rho = 0.108$ — below every per-type value it aggregates. This is why we report every evaluation on a single stratum, and why pooled critical-set figures should be read as inflated wherever they appear. A global sensitivity sweep of $I_{\text{comp}}$’s four severity weights ($N = 1{,}000$ Dirichlet draws; Supplementary §S1.2) shows Application correlation exceeding pooled on *all* draws ($\rho \in [0.432, 0.574]$ vs. $[-0.006, 0.269]$), but the strict condition that pooled fall below *every* stratum — including the weakly-correlated Broker — holds on only $34.8\%$ of the simplex. The stratification argument rests on the former, which is weight-invariant. Evaluation of a rule-based anti-pattern catalog on the same benchmark indicates that the catalog flags $93.8\%$ of scored components and therefore does not discriminate, so critical-set identification is delegated to the continuous rankers of §§7.1–7.2.
...
*While the neural forward pass executes in milliseconds ($56\,\text{ms}$ for 2,000 nodes), cold deterministic feature extraction requires up to $239.3\,\text{s}$, dominated by the $O(|V|^2 + |V||E|)$ Connectivity Degradation Index. Cold static gating is thus roughly seventeen times slower than discrete-event simulation ($80.6\,\text{s}$ vs. $4.8\,\text{s}$ on the 520-node Enterprise mesh), refuting the assumption that static analysis is inherently cheaper than simulation without incremental graph caching.*
...
The framing that motivated this analysis — static gating as a low-cost substitute for dynamic simulation — does not survive measurement against our own oracle. Timing the cascade reachability labeling sweep (five seeds, node types Application/Broker/Library, the full ground-truth run) on the same corpus, the same machine and the same measurement session as the gate gives $0.09$–$4.82\,\text{s}$ per scenario, against $0.04$–$80.6\,\text{s}$ for the analysis gate. Both maxima occur in the 520-component Enterprise mesh, so the largest scenario compares $4.8\,\text{s}$ of simulation against $80.6\,\text{s}$ from static analysis: the gate costs roughly seventeen times as much as the simulation it is meant to replace. These are wall-clock figures on one commodity CPU and should be read to one significant figure: repeating the oracle sweep in a second session reproduced the maximum to within $0.12\,\text{s}$ ($16.7\times$ and $17.2\times$), while the gate maximum has ranged over $78$–$83\,\text{s}$ across sessions on identical work. An earlier pairing reported $11.5\times$; its oracle half carried no provenance stamp, so the machine and commit that produced it cannot be recovered, and it is superseded rather than reconciled.
...
**Conclusion Validity.** Distributions are evaluated using non-parametric correlations (Spearman $\rho$, Kendall $\tau$), bootstrap intervals ($B = 2{,}000$), and Wilcoxon signed-rank tests. Analyses are stratified to prevent Simpson’s paradox (pooled $\rho = 0.108$ vs. per-type $0.119$–$0.571$). Because folds share training graphs, test observations exhibit positive dependence; nominal Wilcoxon $p$-values are optimistic and interpreted descriptively. Primary non-parametric evidence rests on 12-fold sign consistency, though sign tests over overlapping splits inherit positive dependence as well. Furthermore, synthetic graphs derive from a single generator family, bounding empirical generalizability.
