...
Two evaluations are conducted: first, the closed-form explanation layer $Q(v)$ against $I_{\text{comp}}(v)$ ($\rho = 0.514$–$0.800$, exceeding degree centrality); second, §7.4.1 evaluates learned predictors zero-shot against $I^*(v)$. `Topo-QoS` was previously omitted from Table 8 due to raw-multigraph betweenness computation (corrected in §8.4); standard middleware default contract profiles (e.g., ROS 2 Best-Effort/Reliable, MQTT QoS 0/1) were applied uniformly across edges lacking explicit manifests to ensure identical graph representations across baselines.

### 7.4.1 Zero-Shot Transfer of the Learned Model

To assess generalization outside the generator, `HGT-QoS` was trained on all twelve synthetic scenarios and evaluated zero-shot across five open-source systems versus $I^*(v)$ (five seeds, within-graph rank normalization, two layers, 150 epochs; no open-source graphs contributed gradients or checkpoint selection). Table 8 reports the transfer performance.
...
4.  **Critical-set identification reveals a decisive $2.08\times$ learned model advantage.** On operational triage ($F_1@K$), `HGT-QoS` averages $0.517$, more than double structural baselines ($0.248$). $F_1$ exposes vulnerabilities hidden by global correlation: on EdgeX Foundry, structural baselines score $F_1 = 0.000$ ($\rho \approx 0.47$–$0.53$), while `HGT-QoS` reaches $0.500$. In EdgeX’s IoT topology, symmetric star connections from peripheral adapters to brokers produce identical betweenness ties that collapse triage at $K = 20\%$ ($K=4$), whereas relational attention breaks symmetry via message flows (robust across $K \in \{10\%, 20\%, 30\%\}$). On Autoware.universe, `HGT-QoS` improves critical capture by $+280\%$ ($0.633$ vs. $0.167$), and on Cloud Microservices it attains $F_1 = 0.400$ vs. $0.250$.
