...
To assess generalization outside the generator, `HGT-QoS` was trained on all twelve synthetic scenarios and evaluated zero-shot across five open-source systems versus $I^*(v)$ (five seeds; evaluated in an exploratory 2-layer, 150-epoch configuration with within-graph rank normalization; no open-source graphs contributed training gradients or checkpoint selection). Table 11 reports the transfer performance, visually partitioned by communication paradigm into asynchronous pub-sub architectures and synchronous microservice call trees.
...
**External Validity.** The benchmark corpus comprises 2,812 components across seventeen system architectures deliberately spanning six distinct architectural domains: robotics (autonomous driving / ROS 2), smart cities, healthcare integration, financial trading, enterprise application integration (centralized ESB broker hubs and microservices), and industrial/edge IoT (EdgeX Foundry, Home Assistant). This cross-domain diversity was intentionally curated to establish external validity across divergent structural topologies, interaction patterns, and failure cascade dynamics, preventing empirical conclusions from reflecting an idiosyncratic software monoculture. While this deliberate heterogeneity reveals an architectural boundary condition on the active stratum—pub-sub cascades propagate forward via starvation, whereas synchronous RPC microservices cascade backward along invocation trees via timeouts [48]—it ensures that our findings regarding baseline competitiveness and inductive biases are grounded across varied software engineering contexts. Zero-shot transfer to active components reflects this boundary: `HGT-QoS` maintains positive transfer across asynchronous pub-sub systems ($\rho_{>0} \in [+0.262, +0.770]$, subtotal mean $+0.516$, Table 11), while synchronous call trees invert (subtotal mean $-0.071$) and all training-free baselines turn negative on active nodes (Table 12). GNN directional biases must therefore be conditioned on communication synchrony. Scaling beyond 2,000 nodes requires incremental caching or mini-batching (GraphSAINT [92]).
