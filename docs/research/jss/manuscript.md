...
Evaluation across twelve inductive scenarios and five open-source systems reveals three primary empirical boundaries. First, the learned model does not achieve a statistically significant ranking advantage over an unparameterized QoS-weighted centrality baseline ($\rho = 0.638$ vs. $0.553$, $p = 0.151$). Second, relational typing and continuous QoS encodings act as empirical substitutes rather than complements (interaction $-0.199$, negative on 12/12 folds, $p = 0.0005$), providing redundant relational cues against a predominantly topological simulation target. Third, zero-shot transfer declines to $\rho_{>0} = +0.265$ on actively propagating components, inverting on synchronous RPC call trees. Computationally, while neural inference requires only $56\,\text{ms}$, deterministic feature extraction renders cold static analysis eleven times slower than direct discrete-event simulation ($82.7\,\text{s}$ vs. $7.2\,\text{s}$). These findings establish that unparameterized QoS-weighted centrality provides a competitive training-free baseline, that relational typing confers diminishing returns when QoS is present, and that topological feature extraction forms the principal computational bottleneck in pre-deployment static analysis.
...
**Table 7.** In-distribution held-out evaluation across all twelve distributed architecture scenarios, reporting both Spearman rank correlation ($\rho$) and critical-set identification ($F_1@K$, with $K = \text{round}(0.20 \cdot n)$): mean over five seeds; $n$ = held-out Application test count. Substrates differ in-distribution and are listed per predictor in Table 6 (§6.2). Each seed redraws the 60/20/20 split and model initialization.
...
*Full-population transfer ($\rho = 0.767$) is inflated by separating inert components from active ones. On the active stratum the mean drops to $+0.265$ and both synchronous microservice call trees invert, while the three pub-sub systems hold up. We therefore report RQ4 as a negative result: learned relational transfer to authentic open-source architectures is not established.*
...
RQ5 quantifies computing overhead and sustainability during CI/CD evaluation, with per-stage latencies reported in Table 12:
...
-   **Ingestion boundary.** Deployment must condition on communication synchrony (Table 13): on asynchronous pub-sub systems (ROS 2, DDS, MQTT, Kafka) `HGT-QoS` transfers positively on active components, whereas on synchronous RPC/REST meshes it inverts and should not be deployed — use `Topo-QoS` ($\rho = 0.888$) or static call-graph reachability tools instead.
...
|:-----------------------------|:-----------------------------------------------------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------|
| **Communication synchrony**  | Directional bias must match physical failure flow: pub-sub starves forward, synchronous call trees time out backward [48]. | Pub-sub $\rho_{>0} \in [+0.304, +0.704]$; RPC inverts ($-0.029$ Microservices, $-0.213$ Train-Ticket).                                  |
