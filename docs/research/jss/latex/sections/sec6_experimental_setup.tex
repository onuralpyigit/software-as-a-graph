...
\centering
\small
\caption{Predictors reported in this paper. \texttt{-QoS}: QoS-weighted distances (Topo) or the 16-D QoS edge vector (GNNs); \texttt{-P}: trained on the dependency graph; \texttt{Hybrid-X}: engine X corrected by the \texttt{Topo-QoS} prior (Hybrid-GAT-P: by the \texttt{InDeg} prior). Every predictor run in the study, with the earlier names used by the registered plan and the artifacts: Supplementary Section~\ref{S-supp:taxonomy}.}
...
\textbf{GAT-P-QoS} & App--Lib \texttt{DEPENDS\_ON} & Homogeneous & 16-D QoS Vector & 429{,}992 & Yes & \texttt{GAT-QoS} on the dependency graph \\
\textbf{Hybrid-GAT-P} & App--Lib \texttt{DEPENDS\_ON} & Homogeneous & 16-D QoS Vector & 431{,}433 & Yes & \texttt{GAT-P-QoS} + \texttt{InDeg} prior \\
\bottomrule
\end{tabular}%
}
\end{table}
