...
\begin{enumerate}
    \item \emph{In-Distribution Evaluation (Table~\ref{tab:5}):} Evaluated on the seven core synthetic domains from Table~\ref{tab:4} using stratified 60\% train / 20\% validation / 20\% test node splits over five random seeds. This regime is deliberately the narrower of the two: it exists to establish the contrast against the inductive setting --- what typing buys when train and test are drawn from one architecture --- and not to support a generalization claim. The extended domains and the ATM scenario therefore enter only through LOSO, which is where the paper's generalization results are made.
