...
\noindent\textbf{What each learned engine can see.} Every relation of the native multigraph points away from Applications (to Topics, Nodes and Libraries), and the untyped GATs aggregate along edge direction only. In \texttt{GAT}, \texttt{GAT-QoS} and Hybrid-GAT an Application therefore receives no messages, and its score is a function of its own feature vector: on the trained checkpoints, deleting every edge leaves every Application prediction unchanged. HGT reaches Applications through its reverse-direction pass, so an Application's prediction depends on $35$--$59\%$ of its graph; without that pass, as in the registered directionality control \texttt{HGT-QoS-U}, HGT scores Applications per node as well. We report the untyped arms as registered and use this property as a control (Section~\ref{sec:rq2}). Alongside them we report the feature-only control declared in Amendment~3, \texttt{GBM-Feat}: gradient boosting (scikit-learn defaults, one model per entity type) on the identical feature tensors, trained on the same graphs and per-graph label transform, with no graph model at all.
...
\textbf{GAT} & Native Multigraph & Homogeneous & None & 437{,}496 & Yes & Untyped, no QoS; per-node at Applications \\
\textbf{GAT-QoS} & Native Multigraph & Homogeneous & 16-D QoS Vector & 429{,}992 & Yes & Untyped SaG learned engine; per-node at Applications \\
...
\midrule
\multicolumn{7}{l}{\textit{Attribution and directionality controls (Section~\ref{sec:rq2})}} \\
\textbf{GBM-Feat} & Node features only & Per type & None & 100 trees/type & Yes & Feature-only control (Amendment~3) \\
\textbf{GBM-Feat-QoS} & Node features only & Per type & None & 100 trees/type & Yes & \texttt{GBM-Feat} + QoS node columns \\
\textbf{GAT-QoS-nf} & Native Multigraph & Homogeneous & 16-D QoS Vector & 429{,}992 & Yes & \texttt{GAT-QoS} without QoS node columns \\
\textbf{HGT-QoS-U} & Native Multigraph & Heterogeneous & 16-D QoS Vector & 330{,}895 & Yes & \texttt{HGT-QoS} without reverse pass; per-node at Applications \\
\textbf{GAT-w} & Native Multigraph & Homogeneous & Scalar $w(e)$ & 439{,}272 & Yes & Capacity control; per-node at Applications \\
\bottomrule
\end{tabular}%
}
\end{table}
