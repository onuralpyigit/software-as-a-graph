...
SaG's closed-form engine (\texttt{Topo-QoS}), learned engines (\texttt{HGT-QoS}, \texttt{GAT-N-QoS16-C}) and hybrids are compared against unweighted centrality (Topo) and untyped GNNs (Table~\ref{tab:predictor_taxonomy}). All learned predictors ingest the native typed multigraph under LOSO. They differ in typing (relation-specific \texttt{HGTConv} vs.\ untyped GATConv), edge channel (none, scalar $w(e)$, or the 16-D vector) and capacity. The \texttt{-C} controls registered in Amendment~2 match HGT's capacity and edge-channel width. In QoS-off arms every edge weight is 1 and the QoS node columns are zeroed, though four centralities still carry QoS (Section~\ref{sec:3.4}).

\begin{table}[htbp]
\centering
\small
\caption{Predictors reported in this paper. \texttt{-N}: trained on the complete native multigraph; \texttt{-QoS}: consumes declared QoS contracts; \texttt{-C}: widened to HGT's parameter budget. Every predictor run in the study, including in-distribution variants: Supplementary Section~\ref{S-supp:taxonomy}.}
...
\noindent\textbf{Registered analysis plan.} Before the twelve-fold harness produced any result, we registered the primary contrast, \texttt{HGT-QoS} vs.\ \texttt{Topo-QoS}, together with \texttt{HGT} vs.\ \texttt{Topo-QoS} and Holm correction across the two. We call it \emph{registered} rather than pre-registered, because the plan is a file in our repository with no third-party timestamp. Three amendments registered further contrasts, each before its run and each Holm-corrected within its own family: the matched $2\times2$ (Amendment~2), SaG-Hybrid (Amendment~5) and SaG-Hybrid-GAT (Amendment~6). Every other contrast is exploratory. Because the sequence was adaptive (Amendment~6 followed Amendment~2's result), we also pool all eleven registered contrasts under one Holm correction (\path{reproduce/omnibus_holm.py}). Both hybrid primaries remain significant (SaG-Hybrid-GAT $p_{\text{omni}} = 0.016$, SaG-Hybrid $p_{\text{omni}} = 0.034$), and no other registered contrast reaches $\alpha = 0.05$ ($p_{\text{omni}} \ge 0.38$). Supplementary Section~\ref{S-supp:amendments} lists all six amendments with dates and outcomes.
