...
To operationalize this concurrent deployment, SaG implements a \emph{Dual-Engine Consensus Protocol} (available via \path{saag predict --dual}) that runs \texttt{HGT-QoS} and \texttt{Topo-QoS} in parallel over each candidate manifest. The protocol partitions evaluated components into two actionable operational sets:
\begin{align}
