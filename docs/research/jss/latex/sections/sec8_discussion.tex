...
To operationalize this concurrent deployment in continuous integration, SaG defines a \emph{Dual-Engine Consensus Protocol} that runs \texttt{HGT-QoS} and \texttt{Topo-QoS} in parallel over each candidate manifest. The protocol partitions candidate components $V_{\text{app}}$ into two actionable operational sets:
\begin{align}
...
\paragraph{An incomplete baseline on real-world systems} In the benchmark reported in Table~\ref{tab:9b}, \texttt{Topo-QoS} was omitted because its projection-graph cache was not preserved at the time of study execution; we evaluated \texttt{Topo} directly to maintain complete artifact reproducibility. As formalized in Section~\ref{sec:6.2}, the framework now natively derives the application flow projection $G_{\text{flow}}$ directly from heterogeneous dependency paths without requiring cached external projections, enabling seamless zero-shot evaluation of both structural baselines across arbitrary real-world manifests.
