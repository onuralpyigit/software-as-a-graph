...
\noindent\textbf{Declared message criticality is available but withheld by design.}
\texttt{FailureSimulator} can additionally blend an author-declared \texttt{Topic.criticality}
label into its severity term (a $50/50$ blend with $w(t)$), and ships with this disabled: enabling
it would let an author-declared criticality label leak into the oracle used to score predictors
built from that same label space, which our leakage guard forbids.
