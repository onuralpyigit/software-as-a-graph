...
All results are reported on the Application population ($V_{\text{app}}$) against the primary oracle $I^*(v)$, under the input--label independence guarantee (Section~\ref{sec:4.4}). Per-fold results, secondary strata and extended protocol notes are in the Supplementary Material and the experiment pages of the replication repository (Section~\ref{sec:6.1}).
...
\centering
\small
\caption{Inductive LOSO evaluation, Application population, twelve folds. Learned variants share training set, the native multigraph substrate (\texttt{-N}), depth and selection rule (Section~\ref{sec:6.2}). Fold score = mean over five seeds; \textbf{Fold $\sigma$} = spread of the twelve fold means; \textbf{Seed $\sigma$} = median within-fold spread (zero for deterministic baselines); CI = bootstrap over folds ($B = 2{,}000$). \textbf{$\Delta\rho$} is paired by fold against the registered comparator \texttt{Topo-QoS}, with its own bootstrap interval; for every learned variant the interval spans zero.}
\label{tab:7}
\resizebox{\linewidth}{!}{%
...
\noindent\textbf{The QoS-aware projection is the largest single gain.} Re-weighting shortest paths by declared QoS contracts lifts closed-form ranking from $\rho = 0.349$ to $0.553$ on every held-out architecture. Learned engines without the QoS channel reach only about this level (\texttt{HGT} $0.551$; capacity-matched untyped GAT $0.563$, Section~\ref{sec:rq2}), so the representation carries much of the signal.
...
Each hybrid takes a learned engine and gives it one extra input per Application and Library: the \texttt{Topo-QoS} score, rank-normalized within the graph. Its output adds a learned correction to that score on the logit scale, $\hat{I}^*(v) = \sigma\big(z(v) + \alpha\,\operatorname{logit}(p(v))\big)$, with a single learnable $\alpha$. Architecture, loss, epochs, early stopping and seeds are otherwise identical to the underlying engine. \textbf{SaG-Hybrid} is built on \texttt{HGT-QoS} (Amendment~5; $321$ extra parameters). \textbf{SaG-Hybrid-GAT} is built on \texttt{GAT-N-QoS16-C}, the capacity-matched untyped GAT with the QoS channel (Amendment~6; $1{,}441$ extra parameters). Each was registered with its contrasts and decision rule before any run, with no setting tuned. Each was evaluated in its own CPU sweep, with its comparators re-run in the same invocation. The comparator rows are bit-identical across these sweeps, and \texttt{HGT-QoS} reaches $0.622$ on CPU against $0.638$ on GPU.
...
\noindent\textbf{The prior removes the learned engines' failure mode.} On Enterprise, \texttt{HGT-QoS} and \texttt{GAT-N-QoS16-C} score $0.426$ and $0.407$ against \texttt{Topo-QoS}'s $0.795$; with the prior they reach $0.735$ and $0.768$. Telecom RAN turns from a loss into a win for both.
...
\paragraph{Summary} \emph{With model capacity and edge-channel width matched, the QoS edge channel is what improves learned ranking ($+0.073$ main effect, 10 of 12 folds), and relation-specific weights add nothing beyond it (typing main effect $-0.014$, interaction $+0.001$). A capacity-matched untyped GAT with the QoS channel ($\rho = 0.635$) performs as well as \texttt{HGT-QoS} ($0.622$).}

In Table~\ref{tab:7}, the untyped arms have $28{,}168$ parameters against $434{,}620$ for HGT, and they read a 1-dimensional edge channel against HGT-QoS's 16 dimensions. A $2\times2$ over those four arms therefore credits typing with a large gain ($+0.234$ without QoS). That gain is an effect of capacity and channel width (Supplementary Section~\ref{S-supp:naive-2x2}). The control registered in Amendment~2 removes both differences. \texttt{GAT-N-C} and \texttt{GAT-N-QoS16-C} are untyped GATs widened to HGT's parameter budget ($437{,}496$ and $429{,}992$). The latter reads the same 16-D edge channel as \texttt{HGT-QoS}, including the relation one-hot as an edge feature. All four matched arms ran in one CPU sweep, and the decision rule was fixed before any control result existed (Table~\ref{tab:contrasts_matched}).

\begin{table}[htbp]
\centering
\small
\caption{The $2\times2$ with capacity and edge-channel width matched (Amendment~2): \texttt{GAT-N-C} ($\neg$T$\neg$Q, $437{,}496$ parameters), \texttt{HGT} (T$\neg$Q, $434{,}620$), \texttt{GAT-N-QoS16-C} ($\neg$TQ, $429{,}992$), \texttt{HGT-QoS} (TQ, $434{,}620$). One CPU sweep, twelve LOSO folds, five seeds, Application population. Holm correction across the three orthogonal quantities; simple effects are descriptive. Cell means: $0.563$, $0.548$, $0.635$, $0.622$.}
