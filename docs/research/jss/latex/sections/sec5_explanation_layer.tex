...
R(v) = r_{\text{FT}} \cdot FT(v) + (1 - r_{\text{FT}}) \cdot A(v), \quad r_{\text{FT}} = 0.36
    ...
    The intra-dimension weights apply $\lambda = 0.70$ shrinkage blending with a uniform prior. Because comparison matrices are rank-one by construction, these weights are documented conventions rather than independently elicited consensus. The anti-predictive nature of the elicited AHP weights against dynamic simulation (Section~\ref{sec:7.3}) highlights a key conceptual distinction: AHP pairwise comparisons encode human engineering judgments of structural quality and architectural technical debt, whereas dynamic simulation reflects raw message reachability and graph connectivity.
