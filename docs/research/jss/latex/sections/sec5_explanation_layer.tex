...
\begin{itemize}\tightlist
    ...
    \item \textbf{Reliability ($R(v)$):} Blends Fault Tolerance and Availability: $R(v) = r_{\text{FT}} \cdot FT(v) + (1 - r_{\text{FT}}) \cdot A(v)$ with $r_{\text{FT}} = 0.36$. Intra-dimension weights apply $\lambda = 0.70$ shrinkage blending with a uniform prior. Because comparison matrices are rank-one by construction, their Consistency Ratios are uninformative (see Supplementary Section~S4). Elicited AHP weights rank worse than a uniform prior against dynamic simulation (Section~\ref{sec:7.3}); pending formal user studies, we explicitly recommend the uniform prior.
