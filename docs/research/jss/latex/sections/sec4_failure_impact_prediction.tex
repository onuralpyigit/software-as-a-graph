...
\begin{itemize}
    ...
    \item \textbf{Dynamic Queue-Flow Oracle ($I_{\text{dyn}}(v)$)}, evaluated via discrete-event message-flow queue simulation (built on SimPy \cite{team2020simpy}): simulates emission rates, stochastic latencies, broker buffer saturation, and queue drops under fault injection, extracting the drop in delivered message rate to surviving consumers. It serves only as an independent convergent-validity probe (Section~\ref{sec:7.3.convergent}).
