...
\begin{itemize}
    \item \textbf{Cascade Reachability Oracle ($I^*(v)$):} Implemented via \texttt{FaultInjector}, it crashes component $v$, propagates outages across dependent topics, brokers and links by breadth-first traversal, and returns the fraction of surviving subscriber feeds severed. Publisher loss is weighted by publication rate and feed losses scaled by a QoS ladder ($\times 1.2$ \texttt{RELIABLE}, $\times 1.15$ high/urgent priority, $\times 1.05$ medium) before clamping to $[0,1]$. This is the \textbf{primary continuous target label} throughout.
