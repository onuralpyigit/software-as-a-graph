...
The outer split $(\alpha_{\text{top}}, \beta_{\text{top}}, \gamma_{\text{top}})$ is a convex combination. Sweeping the full simplex leaves induced orderings intact ($\rho \ge 0.919$), moving downstream ranking by only $0.031$ (\texttt{Topo-QoS}) and $0.007$ (RM). Under Morris screening none of the three constants is load-bearing ($\mu^* \le 0.025$). The split is therefore a documented convention rather than a sensitive parameter.
...
Rule 6 is the only symmetric rule: two brokers colocated on the same host share that host's physical failure domain ($w = w_V(\text{host})$). Colocated brokers compete for resources, and host crashes halt them simultaneously. Rule 6 applies in four of eight detection benchmark contexts (12 directed edges). Because simulation oracles operate strictly on $G_{\text{structural}}$ (Section~\ref{sec:4.4}), Rule 6 has zero influence on ground-truth failure labels.
