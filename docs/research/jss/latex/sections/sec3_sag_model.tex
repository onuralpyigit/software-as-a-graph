...
\begin{enumerate}
    \item \textbf{Structural Graph ($G_{\text{structural}}$):} The raw graph containing physical and structural edges (\texttt{PUBLISHES\_TO}, \texttt{ROUTES}, \texttt{RUNS\_ON}, \texttt{USES}). This view is consumed exclusively by discrete-event simulators to execute unbiased failure injections (Section~\ref{sec:4.3}).
...
Both pathways read the same typed node features off $G_{\text{analysis}}$: the predictor (Section~\ref{sec:4}) projects them per entity type before message passing, and the explanation layer (Section~\ref{sec:5}) aggregates them into its quality profile. SaG extracts feature vectors tailored to the 5 entity types:
\begin{itemize}
