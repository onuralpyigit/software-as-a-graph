...
Application and Library entities also incorporate static code metrics generated by Static Code Analysis (SCA) tools (\texttt{cm\_*} attributes: lines of code, cyclomatic complexity, coupling between objects, LCOM), thereby linking code-level fragility directly to topological analysis.
...
The outer split $(\alpha_{\text{top}}, \beta_{\text{top}}, \gamma_{\text{top}})$ constitutes a declared convex combination. Comprehensive sensitivity analysis using Morris elementary effects (Supplementary Section~S1) demonstrates that topic-weight variation alters downstream rank correlation by no more than $0.018$ (with $\rho = 0.081$ maximum shift across the full simplex). This finding indicates that the split is a documented convention rather than a sensitive parameter.
...
The SaG framework comprises two distinct representations: (1)~\textbf{Structural Graph} ($G_{\text{structural}}$), which is the raw deployment graph containing physical and structural relations (\texttt{PUBLISHES\_TO}, \texttt{ROUTES}, \texttt{RUNS\_ON}, \texttt{USES}) and preserves the untransformed deployment topology; and (2)~\textbf{Analysis Graph} ($G_{\text{analysis}}$), which is the projected graph containing derived \texttt{DEPENDS\_ON} edges annotated with QoS weights and ingested SCA metrics. All GNN embeddings and analytical metrics are computed on the analysis graph (see running example in Supplementary Figure~S14.1).

$G_{\text{analysis}}$ is further organized into four analytical layers (Application, Middleware, Infrastructure, and Global System), which enables evaluation of criticality at subsystem levels in a manner consistent with hierarchical frameworks such as MIL-STD-498 \cite{dod1994milstd498}.
