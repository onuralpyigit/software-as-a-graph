...
This section formalizes the Software-as-a-Graph multigraph representation (Section~\ref{sec:3.1}), the QoS-aware weighting and logical dependency derivation rules (Section~\ref{sec:3.2}), the dual graph views (Section~\ref{sec:3.3}), and the typed node feature representations (Section~\ref{sec:3.4}).
...
Application and Library entities additionally incorporate static code metrics computed via Static Code Analysis (SCA) tools (\texttt{cm\_*} attributes: lines of code, cyclomatic complexity, coupling between objects, LCOM), thereby linking code-level fragility to topological analysis.

\subsection{QoS-Aware Weights and Logical Dependency Derivation}
\label{sec:3.2}

In distributed middleware, communication links vary in coupling strength based on their Quality-of-Service (QoS) contracts. For example, a \texttt{RELIABLE} topic with \texttt{TRANSIENT\_LOCAL} durability binds communicating services more tightly than a \texttt{BEST\_EFFORT} telemetry stream.
