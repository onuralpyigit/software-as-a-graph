...
\caption{End-to-end architecture of the SaG framework. The central assertion is that the predictive pathway (Section~\ref{sec:4}) forms the primary sequence: manifest ingestion $\to$ typed multigraph $\to$ QoS-weighted \texttt{DEPENDS\_ON} projection $\to$ typed node properties $\to$ heterogeneous graph learning $\to$ a ranked critical set with per-relationship criticality $\to$ the ground-truth simulation oracle (Section~\ref{sec:4.3}) that evaluates it. The oracle completes the predictive pathway's training loop and operates solely on $G_{\text{structural}}$; it functions strictly offline and is excluded from inference (Section~\ref{sec:4.4}), as indicated by the dashed edge. The explanation layer (Section~\ref{sec:5}) differs from this sequence: it re-enters from the analysis multigraph, generates a standards-based quality profile from the same typed features without sharing parameters with the predictor, and is accessed through triage instead of direct data flow.}
...
Rule 6 is the only symmetric projection rule: two brokers colocated on the same execution host share that host's physical failure domain (following the simultaneous-blast principle of Rule 5, with $w = w_V(\text{host})$). In production deployments, colocated brokers compete for CPU, memory, and I/O; a host crash halts all colocated instances simultaneously. Rule 6 does not model logical intra-cluster broker coupling (e.g., partition replication, quorum election, or shovel links, which do not require physical colocation). It applies in four of the eight detection benchmark contexts (Supplementary Section~S6; the subset itself is tabulated in Section~S12) and contributes only 12 directed edges. Because simulation oracles operate strictly on $G_{\text{structural}}$ (Section~\ref{sec:4.4}), Rule 6 has zero influence on ground-truth failure labels.
