...
* **9.5 Computational Overhead and CI/CD Feasibility (RQ5):** Measured gate times 0.03–24.6 s (app layer) / 0.16–33.0 s (system layer). **The active detector count and its ≈0.2 s total across all eight scenarios** — TODO(needs re-measurement): the catalog dropped from 21 to 19 patterns with TARGET/EXPOSURE retired (§4.3); the "twenty active detectors" figure predates that and has not been recounted against the current `saag/analysis/antipattern_detector.py` catalog — the entire budget is the analysis stage, so optimizing detectors buys nothing. **`DEEP_PIPELINE` is excluded as a defect**: it enumerates every simple source-to-sink path, producing 247,761 findings on a 29-component fixture and failing to terminate on a 50-application topology. The earlier generate–verify loop runtimes (4.7 s–649.6 s) are **withdrawn** as pre-filter measurements and replaced by the per-edit cost model.
...
* **10.5 External Validity:** One generator, synthetic topologies; catalog scoped to pub-sub; acceptance rate established over 98–326 components only; detection figures characterize a TODO(needs re-measurement): "twenty-detector" catalog figure predates TARGET/EXPOSURE's retirement (catalog is now 19 patterns, §4.3) and needs recounting before restating.
