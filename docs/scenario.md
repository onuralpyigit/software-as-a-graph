...
| `scenario_14_realworld_homeassistant.yaml` | `realworld_homeassistant.json` | `homeassistant_iot` | 24 / 22 / 3 / 6 / 8 | 2026 | `8e47feee0cd4` |
| `scenario_15_realworld_edgex.yaml` | `realworld_edgex.json` | `edgex_foundry_iiot` | 22 / 24 / 3 / 6 / 8 | 2026 | `417df24244ba` |

These five datasets represent authentic, real-world open-source software architectures converted into canonical SaG typed multigraph format via [`saag.adapters.realworld_adapter.RealWorldAdapter`](../saag/adapters/realworld_adapter.py) and generated via [`cli/import_realworld_system.py`](../cli/import_realworld_system.py):
...
* **`realworld_homeassistant.json` (`homeassistant_iot`)**: Authentic Home Assistant smart home IoT topology ([Home Assistant](https://www.home-assistant.io/), open-source local-first home automation). Spans 24 Applications (core state machine, automation rule engine, script runner, safety hazard monitor, alarm control panel, emergency valve/siren actuation, ZHA Zigbee gateway, Z-Wave supervisor, ESPHome manager, MQTT bridge, camera stream manager, climate HVAC controller, recorder persistence worker, energy analytics, WebSocket server, HTTP ingress, Lovelace UI dashboard), 22 Topics with event-bus and MQTT QoS profiles, 3 Brokers (AsyncIO EventBus, Mosquitto MQTT, WebSocket push router), 6 Deployment Nodes (Appliance host, Zigbee USB coordinator, Z-Wave hub, TrueNAS storage, ESPHome bridge, Wall tablet kiosk), and 8 shared Python libraries (`aiohttp`, `voluptuous`, `sqlalchemy`, `paho-mqtt`, `zeroconf`, etc.).
* **`realworld_edgex.json` (`edgex_foundry_iiot`)**: Industrial EdgeX Foundry IoT edge computing topology ([LF Edge EdgeX Foundry](https://www.edgexfoundry.org/)). Contains 22 Applications across Southbound Device services (Modbus, MQTT, GPIO, Camera Vision, SNMP), Core services (Core Data, Metadata, Command, Keeper Registry), Northbound App & Analytics services (eKuiper stream rules engine, configurable app service, safety interlock, AWS/Azure cloud exporters, SCADA historian persister), and Support/Security services (Kong proxy, Vault secret store, notifications, scheduler). Spans 24 Topics with industrial QoS configurations, 3 Brokers (Redis Streams bus, Mosquitto OT field broker, eKuiper stream bus), 6 Deployment Nodes (industrial gateway, factory cell PLCs, SCADA historian, DMZ security appliance, Jetson Orin accelerator), and 8 shared Go modules (`go-mod-core-contracts`, `go-mod-messaging`, `go-mod-bootstrap`, etc.).
...
| Real-world system benchmarks | `realworld_autoware_ros2.json`, `realworld_cloud_microservices.json`, `realworld_trainticket.json`, `realworld_homeassistant.json`, `realworld_edgex.json` | `cli/import_realworld_system.py`, `examples/run_autoware_ros2_pipeline.py`, `tests/test_realworld_adapter.py` |
...
and G3 Precision@K ≥ 0.80 (see [validation.md §5.1](validation.md#61-library-gate-suite-validationservice) for the
