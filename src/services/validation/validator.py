...
class Validator:
    ...
    def _validate_group(
        self, group_name: str, predicted: Dict[str, float], actual: Dict[str, float], types: Dict[str, str]
    ...
    def _empty_result(self, timestamp, layer, context, pc, ac, mc, warnings) -> ValidationResult:
        ...
        empty_group = ValidationGroupResult(
            ...
            classification=ClassificationMetrics(), ranking=RankingMetrics()
        )
        return ValidationResult(
            timestamp=timestamp, layer=layer, context=context, targets=self.targets,
            overall=empty_group, predicted_count=pc, actual_count=ac, matched_count=mc, warnings=warnings
        )
