...
Unlike analysis layers (which use DEPENDS_ON), simulation layers work on:
- PUBLISHES_TO, SUBSCRIBES_TO, ROUTES, RUNS_ON, CONNECTS_TO, USES

Layers:
    - app: Simulate Applications via PUBLISHES_TO/SUBSCRIBES_TO/USES
    - infra: Simulate Nodes via RUNS_ON/CONNECTS_TO
    - mw: Simulate Brokers via ROUTES/PUBLISHES_TO/SUBSCRIBES_TO
    - system: Simulate all components with all relationships
"""
...
class SimulationLayer(Enum):
    """
    Enumeration of simulation layers for multi-layer graph model.
    
    Each layer represents a specific view of the distributed system:
    - APP: Application-level message flow (analyze Applications)
    - INFRA: Infrastructure topology (analyze Nodes)
    - MW: Middleware message routing (analyze Brokers)
...
class SimulationLayerDefinition:
    ...
    Attributes:
        ...
        component_types: Types of vertices included in simulation graph
        analyze_types: Types of components to analyze/report (subset of component_types)
        relationships: RAW relationship types to traverse during simulation
        cascade_rules: Cascade rule types for failure simulation
...
# Layer definitions with specific RAW relationship types
...
def get_layer_definition(layer: SimulationLayer) -> SimulationLayerDefinition:
    """Get the definition for a specific layer."""
    return SIMULATION_LAYERS[layer]


def get_all_layers() -> List[SimulationLayer]:
    """Get all available simulation layers."""
    return list(SimulationLayer)


def get_primary_layers() -> List[SimulationLayer]:
    """Get the primary layers for standard simulation."""
    return [SimulationLayer.APP, SimulationLayer.INFRA, SimulationLayer.MW, SimulationLayer.SYSTEM]


def list_layers() -> str:
    """Return a formatted string describing all available layers."""
    lines = ["Available simulation layers:", ""]
    for layer in SimulationLayer:
        defn = SIMULATION_LAYERS[layer]
        rels = ", ".join(sorted(defn.relationships))
        analyze = ", ".join(sorted(defn.analyze_types))
