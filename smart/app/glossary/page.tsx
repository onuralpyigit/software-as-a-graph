...
const ALL_TERMS: Term[] = [
  ...
  { term: "Analysis Layer", definition: "The view-scope filter applied to the graph in the Analysis page. Four layers: app (Application + Library nodes), infra (Node edges), mw (Broker edges), system (all component types). Determines which DEPENDS_ON subtypes are included in RM scoring." },
  { term: "Anti-Pattern", definition: "An architectural smell detected by the AntiPatternDetector after RM scoring. Severity levels: CRITICAL, HIGH, MEDIUM. Types: SPOF, BRIDGE_EDGE, BOTTLENECK_EDGE, BROKER_OVERLOAD, FAILURE_HUB, CONCENTRATION_RISK, SYSTEMIC_RISK, DEEP_PIPELINE, TOPIC_FANOUT, QOS_MISMATCH, GOD_COMPONENT, HUB_AND_SPOKE, CHATTY_PAIR, ORPHANED_TOPIC, UNSTABLE_INTERFACE, CYCLE, CHAIN, ISOLATED, COMPOUND_RISK. (TARGET and EXPOSURE were retired with the Vulnerability/Security dimension.)" },
  { term: "Availability Score A(v)", definition: "Structural impact of removing component v on graph connectivity. Not a top-level dimension of Q(v) — a sub-characteristic of Reliability: R(v) = α·FT(v) + (1−α)·A(v), α = 0.36.", formula: "A(v) = 0.35·AP_c_directed + 0.25·QSPOF + 0.25·BR + 0.10·CDI + 0.05·w(v)" },
  // B
  ...
  { term: "criticality", definition: "Mission-criticality flag on Application and Library nodes (e.g., 'critical'). Drives the Criticality I/O and Node Density Statistics tabs and feeds the RM scoring pipeline. Blank values are treated as non-critical." },
  ...
  { term: "Cascade Depth Potential", definition: "Estimated maximum cascade depth for a component — the longest failure chain it can initiate. Enhanced variant multiplies by (1 + MPCI) to account for parallel cascade paths. Input to FT(v), Reliability's fault-tolerance sub-characteristic.", formula: "CDPot_enh = CDPot_base × (1 + MPCI)" },
  { term: "Classification Level", definition: "Five-tier severity classification applied to each RM dimension score: CRITICAL (> Q3 + 0.75×IQR), HIGH (> Q3), MEDIUM (> Median), LOW (> Q1), MINIMAL (≤ Q1). For samples < 12 a fixed-percentile fallback is used." },
  ...
  { term: "IA(v)", definition: "Availability ground-truth impact — QoS-weighted connectivity disruption from removing component v. Feeds IR(v) via the α-blend alongside IFT(v); also reported standalone as a diagnostic.", formula: "IA(v) = 0.50·WeightedReachabilityLoss + 0.35·WeightedFragmentation + 0.15·PathBreakingThroughputLoss" },
  ...
  { term: "IFT(v)", definition: "Fault-tolerance ground-truth impact — cascade failure reach and depth when component v fails. Formerly called IR(v); renamed when IR(v) became the α-blend of this and IA(v).", formula: "IFT(v) = 0.45·CascadeReach + 0.35·WeightedCascadeImpact + 0.20·NormalizedCascadeDepth" },
  ...
  { term: "IR(v)", definition: "Reliability ground-truth impact. The α-blend of IFT(v) (fault-tolerance) and IA(v) (availability), mirroring the scoring-side hierarchy R(v) = α·FT(v) + (1−α)·A(v). The simulation label consumed by the GNN Predict stage.", formula: "IR(v) = α·IFT(v) + (1−α)·IA(v),  α = 0.36" },
  // L
  ...
  // M
  { term: "Maintainability Score M(v)", definition: "RM Maintainability dimension score measuring change-propagation risk and structural bottleneck position. Weight in Q(v): 0.20 — derived from the retired 4-D AHP composite, not AHP-fitted directly (a 2-term composite is consistent by construction and AHP adds nothing at that level).", formula: "M(v) = 0.35·BT + 0.30·w_out + 0.15·CQP + 0.12·CouplingRisk + 0.08·(1−CC)" },
  ...
  { term: "Q(v)", definition: "Overall component quality score in the Analysis page, aggregating the two RM dimensions. Weights are derived algebraically from the retired 4-D AHP composite (dropping Vulnerability/Security and renormalising), not AHP-fitted at this level — a 2-term composite is consistent by construction.", formula: "Q(v) = 0.80·R(v) + 0.20·M(v)" },
  ...
  // R
  { term: "Reliability Score R(v)", definition: "RM Reliability dimension score. Hierarchical: a weighted combination of Fault Tolerance FT(v) and Availability A(v), its two sub-characteristics. Weight in Q(v): 0.80.", formula: "R(v) = α·FT(v) + (1−α)·A(v),  α = 0.36" },
  { term: "Fault Tolerance Score FT(v)", definition: "Reliability sub-characteristic measuring fault-propagation reach from a component. Formerly the whole of R(v) under the retired 4-dimension model.", formula: "FT(v) = 0.45·RPR + 0.30·DG_in + 0.25·CDPot_enh" },
  { term: "Reverse PageRank", definition: "PageRank computed on the transposed graph G^T — measures fault-propagation reach from a failed component outward through reverse dependency edges. Primary input to FT(v)." },
  { term: "RM", definition: "The two quality dimensions assessed per component in the Analysis page: Reliability (R, hierarchical over Fault Tolerance and Availability) and Maintainability (M). Vulnerability/Security was retired; each surviving dimension has a dedicated score, ground-truth simulator, and validation metric set." },
  ...
  { term: "Topic Weight", definition: "QoS-based operational priority weight ∈ [0, 1] assigned to a topic. Derived from its reliability and durability QoS settings. 0.0 = minimal priority · 1.0 = highest criticality. Feeds the RM scoring pipeline and Traffic Simulator weighting." },
  ...
  { term: "vulnerabilities", definition: "Number of known security vulnerabilities detected by static analysis (e.g., SonarQube). An optional property on Application and Library nodes; ingested and persisted, but — like cm_avg_cbo and cm_avg_rfc — not read by any RM scoring formula." },
  // W
...
const ABBREVIATIONS: Abbreviation[] = [
  ...
  { abbr: "AHP",  expansion: "Analytic Hierarchy Process",             definition: "Pairwise comparison method used to derive intra-dimension weights (e.g. FT(v)'s RPR/DG_in/CDPot split). A shrinkage factor λ=0.7 blends AHP-derived weights with a uniform prior to prevent over-fitting. Retired at the Q(v) composite level: a 2-term composite is consistent by construction, so AHP contributes nothing there — the R(v)/M(v) composite weights are DECLARED, derived algebraically from the retired 4-D composite.", },
  ...
  { abbr: "CDPot",expansion: "Cascade Depth Potential",               definition: "Estimated maximum cascade depth for a component. Enhanced: CDPot_enh = CDPot_base × (1 + MPCI). Input to FT(v)." },
  ...
  { abbr: "DG_in",expansion: "Normalised In-Degree",                  definition: "In-degree of a component normalised to [0,1]. Direct dependent count; primary input to FT(v) alongside RPR." },
  ...
  { abbr: "GAT",  expansion: "Graph Attention Network",               definition: "The architecture used by the homogeneous baseline variants (gl / gl_qos), which flatten the graph to a single node/edge type before learning attention weights over it." },
  ...
  { abbr: "GNN",  expansion: "Graph Neural Network",                  definition: "The machine-learning model in Step 3 (Predict). An HGT trained on simulate-derived ground-truth labels that generalises beyond closed-form RM scoring." },
  { abbr: "HGT",  expansion: "Heterogeneous Graph Transformer",       definition: "The default GNN architecture used in the Predict stage. Relation-specific attention over typed node and edge types learns criticality from simulated ground-truth labels." },
  ...
  { abbr: "IA",   expansion: "Availability Impact IA(v)",             definition: "Ground-truth availability impact from removing v: QoS-weighted connectivity disruption. Feeds IR(v) as its (1−α) term, alongside IFT(v)." },
  { abbr: "IFT",  expansion: "Fault Tolerance Impact IFT(v)",         definition: "Ground-truth cascade failure impact: CascadeReach, WeightedCascadeImpact, NormalizedCascadeDepth. Feeds IR(v) as its α term." },
  ...
  { abbr: "IR",   expansion: "Reliability Impact IR(v)",              definition: "Ground-truth blended reliability impact: IR(v) = α·IFT(v) + (1−α)·IA(v), α = 0.36. Training label for the GNN Predict stage." },
  ...
  { abbr: "MPCI", expansion: "Multi-Path Cascade Index",              definition: "Measures the number and combined weight of parallel cascade paths from a component. Enhances CDPot: CDPot_enh = CDPot_base × (1 + MPCI). Input to FT(v) with weight 0.25." },
  ...
  { abbr: "RM",   expansion: "Reliability · Maintainability",         definition: "The two quality characteristics scored per component (ISO/IEC 25010:2023). Reliability is itself hierarchical: R(v) = α·FT(v) + (1−α)·A(v), α = 0.36. Composite: Q(v) = 0.80R + 0.20M." },
  { abbr: "RPR",  expansion: "Reverse PageRank",                      definition: "PageRank on G^T — measures fault-propagation reach from a failed component. Input to FT(v) with weight 0.45." },
