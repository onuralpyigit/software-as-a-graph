...
const ALL_TERMS: Term[] = [
  ...
  { term: "Analysis Layer", definition: "The view-scope filter applied to the graph in the Analysis page. Four layers: app (Application + Library nodes), infra (Node edges), mw (Broker edges), system (all component types). Determines which DEPENDS_ON subtypes are included in RMAV scoring." },
  { term: "Anti-Pattern", definition: "An architectural smell detected by the AntiPatternDetector after RMAV scoring. Severity levels: CRITICAL, HIGH, MEDIUM. Types: SPOF, FAILURE_HUB, GOD_COMPONENT, TARGET, BRIDGE_EDGE, EXPOSURE, CYCLE, HUB_AND_SPOKE, CHAIN, SYSTEMIC_RISK." },
  { term: "Availability Score A(v)", definition: "RMAV Availability dimension score measuring the structural impact of removing component v on graph connectivity. The dominant dimension in Q(v) with AHP weight 0.43.", formula: "A(v) = 0.35·AP_c_directed + 0.25·QSPOF + 0.25·BR + 0.10·CDI + 0.05·w(v)" },
  // B
  ...
  { term: "criticality", definition: "Mission-criticality flag on Application and Library nodes (e.g., 'critical'). Drives the Criticality I/O and Node Density Statistics tabs and feeds the RMAV scoring pipeline. Blank values are treated as non-critical." },
  ...
  { term: "Classification Level", definition: "Five-tier severity classification applied to each RMAV dimension score: CRITICAL (> Q3 + 0.75×IQR), HIGH (> Q3), MEDIUM (> Median), LOW (> Q1), MINIMAL (≤ Q1). For samples < 12 a fixed-percentile fallback is used." },
  ...
  { term: "IV(v)", definition: "Vulnerability ground-truth impact — adversarial compromise propagation from v via trust-based dependency edges (θ_trust = 0.30).", formula: "IV(v) = 0.40·AttackReach + 0.35·WeightedAttackImpact + 0.25·HighValueContamination" },
  // L
  ...
  // M
  { term: "Maintainability Score M(v)", definition: "RMAV Maintainability dimension score measuring change-propagation risk and structural bottleneck position. AHP weight in Q(v): 0.17.", formula: "M(v) = 0.35·BT + 0.30·w_out + 0.15·CQP + 0.12·CouplingRisk + 0.08·(1−CC)" },
  ...
  { term: "Q(v)", definition: "Overall component quality score in the Analysis page, aggregating four RMAV dimensions via AHP weights (shrinkage λ=0.7).", formula: "Q(v) = 0.24·R(v) + 0.17·M(v) + 0.43·A(v) + 0.16·V(v)" },
  { term: "QADS", definition: "QoS-weighted Attack-Dependent Surface — inbound dependency weight measuring how many high-criticality components trust this node. Input to V(v)." },
  ...
  { term: "Q_ensemble", definition: "Ensemble-blended quality score combining GNN inference with rule-based RMAV scoring in the Predict stage.", formula: "Q_ensemble(v) = α·Q_GNN + (1−α)·Q_RMAV" },
  // R
  { term: "Reliability Score R(v)", definition: "RMAV Reliability dimension score measuring fault-propagation reach from a component. AHP weight in Q(v): 0.24.", formula: "R(v) = 0.45·RPR + 0.30·DG_in + 0.25·CDPot_enh" },
  { term: "Reverse Closeness Centrality", definition: "Closeness centrality computed on the transposed graph G^T. Measures how quickly an adversarial compromise can propagate from this component to all others via trust-based dependency paths. Input to V(v)." },
  { term: "Reverse Eigenvector Centrality", definition: "Eigenvector centrality computed on the transposed graph G^T. Identifies components that are central in the reverse dependency network, indicating strategic attack reach. Input to V(v)." },
  ...
  { term: "RMAV", definition: "The four quality dimensions assessed per component in the Analysis page: Reliability (R), Maintainability (M), Availability (A), and Vulnerability (V). Each dimension has a dedicated score, ground-truth simulator, and validation metric set." },
  ...
  { term: "vulnerabilities", definition: "Number of known security vulnerabilities detected by static analysis (e.g., SonarQube). An optional property on Application and Library nodes that contributes to vulnerability risk scoring when present." },
  { term: "Vulnerability Score V(v)", definition: "RMAV Vulnerability dimension score measuring adversarial compromise risk via reverse-graph centrality metrics. AHP weight in Q(v): 0.16.", formula: "V(v) = 0.40·REV + 0.35·RCL + 0.25·QADS" },
  // W
...
const ABBREVIATIONS: Abbreviation[] = [
  ...
  { abbr: "AHCR@K",expansion: "Attack Hit Capture Rate at K",          definition: "Fraction of the top-K simulated attack targets (by IV(v)) captured by the top-K vulnerability predictions V(v). Validation metric for the Vulnerability dimension." },
  { abbr: "AHP",  expansion: "Analytic Hierarchy Process",             definition: "Pairwise comparison method used to derive RMAV dimension weights. A shrinkage factor λ=0.7 blends AHP-derived weights with a uniform prior to prevent over-fitting to a single dimension.", },
  { abbr: "APAR", expansion: "Attack Path Agreement Rate",             definition: "Fraction of adversarial propagation paths that agree between IV(v) simulation and V(v) prediction. Validation metric for the Vulnerability dimension." },
  ...
  { abbr: "FTR",  expansion: "False Trust Rate",                      definition: "Fraction of non-compromised components incorrectly predicted as vulnerable by V(v). Validation metric for the Vulnerability dimension." },
  ...
  { abbr: "GNN",  expansion: "Graph Neural Network",                  definition: "The machine-learning model in Step 3 (Predict). A heterogeneous GAT trained on simulate-derived ground-truth labels that generalises beyond closed-form RMAV scoring." },
  ...
  { abbr: "IV",   expansion: "Vulnerability Impact IV(v)",            definition: "Ground-truth adversarial compromise impact via trust-based BFS on G^T with θ_trust = 0.30." },
  ...
  { abbr: "QADS", expansion: "QoS-weighted Attack-Dependent Surface", definition: "Inbound dependency weight measuring high-criticality components that trust this node. Input to V(v) with weight 0.25." },
  ...
  { abbr: "RCL",  expansion: "Reverse Closeness Centrality",          definition: "Closeness centrality on G^T. Measures adversarial propagation speed from a component. Input to V(v) with weight 0.35." },
  { abbr: "REV",  expansion: "Reverse Eigenvector Centrality",        definition: "Eigenvector centrality on G^T. Identifies components with the highest strategic attack reach. Input to V(v) with weight 0.40." },
  { abbr: "RMAV", expansion: "Reliability · Maintainability · Availability · Vulnerability", definition: "The four quality dimensions scored per component: Q(v) = 0.24R + 0.17M + 0.43A + 0.16V." },
